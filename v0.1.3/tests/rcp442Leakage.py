...
def getRun2k11(deal, aHand):
  a1 = genPermAnn(aHand, ann1)[0]
  a2 = SecondAnn(deal, a1)
  a3 = ('c', [deal['c']])
  run1 = [a1, a2, a3]
  return run1
...
def tabs2k11(nRuns, nCols):
  ...
  for i in range(nTabs):
    ...
    currTab = getLatex(lkdLst, currentRunL, 'c', startIdx)
...
def cardsLeakedTabs(lkdLst, runList, agt, startIdx):
  '''
...
def dealsPossibleTabs(lkdLst, runList, agt, startIdx):
  '''
  Given an array of runs, return a table 
...
def getLatex(lkdLst, runList, agt, startIdx):
  sI = startIdx
  tC = cardsLeakedTabs(lkdLst, runList, agt, sI)
  pD = dealsPossibleTabs(lkdLst, runList, agt, sI)
  res = ""
